...
const validateMessage = <Action extends ActionName, T extends "request" | "response">(
...
): Either<ValidationError, ReqRes<Action>[T]> => {
  ...
  if (!result.valid)
    return Left(
      new ValidationError(`jsonschema errors: ${result.errors.map((err) => err.toString())}`),
    );
...
export const validateMessageRequest = <T extends ActionName>(
  ...
  body: object,
  acceptedActions: T[],
): Either<ValidationError, Request<T>> => validateMessage("request", action, body, acceptedActions);

export const validateMessageResponse = <T extends ActionName>(
  action: string,
  body: object,
  acceptedActions: T[],
): Either<ValidationError, Response<T>> =>
